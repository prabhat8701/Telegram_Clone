...
function Chat({ pp, contact, msg, time, unreadMsgs, active }) {
  return (
    // Chat container
    <div
      className={`flex justify-between items-center cursor-pointer w-100 h-[85px] px-3 hover:bg-[dark] ${
        active ? "bg-[dark]" : ""
      }`}
    >
      {/* Profile picture */}
      ...
      <div className="flex justify-between border-t border-neutral-700 w-100 h-100 py-3">
        ...
        <div className="flex flex-col justify-between items-end h-100 text-xs">
          {/* Time */}
          <p className="text-emerald-500 min-w-[55px] text-white">{time}</p>

          {/* Number of messages */}
          {unreadMsgs && (
            <div className="flex justify-center items-center bg-dark rounded-full w-[20px] h-[20px]">
              <p className="text-emerald-900 text-white">{unreadMsgs}</p>
            </div>
          )}
        </div>
      </div>
